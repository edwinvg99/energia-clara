...
/* ─── Colores por métrica ─────────────────────────────────── */
const COLOR_CONFIG = {
  emerald: { bg: 'bg-emerald-500', light: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200', bar: 'bg-emerald-400', badge: 'bg-emerald-100 text-emerald-800' },
  blue:    { bg: 'bg-blue-500',    light: 'bg-blue-50',    text: 'text-blue-700',    border: 'border-blue-200',    bar: 'bg-blue-400',    badge: 'bg-blue-100 text-blue-800'    },
  amber:   { bg: 'bg-amber-500',   light: 'bg-amber-50',   text: 'text-amber-700',   border: 'border-amber-200',   bar: 'bg-amber-400',   badge: 'bg-amber-100 text-amber-800'  },
  cyan:    { bg: 'bg-cyan-500',    light: 'bg-cyan-50',    text: 'text-cyan-700',    border: 'border-cyan-200',    bar: 'bg-cyan-400',    badge: 'bg-cyan-100 text-cyan-800'    },
  rose:    { bg: 'bg-rose-500',    light: 'bg-rose-50',    text: 'text-rose-700',    border: 'border-rose-200',    bar: 'bg-rose-400',    badge: 'bg-rose-100 text-rose-800'    },
  purple:  { bg: 'bg-purple-500',  light: 'bg-purple-50',  text: 'text-purple-700',  border: 'border-purple-200',  bar: 'bg-purple-400',  badge: 'bg-purple-100 text-purple-800' },
  orange:  { bg: 'bg-orange-500',  light: 'bg-orange-50',  text: 'text-orange-700',  border: 'border-orange-200',  bar: 'bg-orange-400',  badge: 'bg-orange-100 text-orange-800' },
  teal:    { bg: 'bg-teal-500',    light: 'bg-teal-50',    text: 'text-teal-700',    border: 'border-teal-200',    bar: 'bg-teal-400',    badge: 'bg-teal-100 text-teal-800'    },
};
...
const METRIC_INFO = {
  ...
  VoluUtilDiarEner: {
    ...
    contexto: 'Es el indicador de "salud hídrica" del sistema. Si el volumen cae por debajo del 30–40% de la capacidad útil, se activan alertas de desabastecimiento, el precio de bolsa sube y se pueden aplicar restricciones.',
  ...
  factorEmisionCO2e: {
    ...
    contexto: 'Colombia tiene uno de los menores factores de emisión eléctrica de América Latina gracias a su matriz predominantemente hidráulica. El valor sube cuando las termoeléctricas de carbón o gas generan más (sequías, fallas en plantas hidráulicas).',
  ...
  ENFICC: {
    ...
    contexto: 'A través del Cargo por Confiabilidad (CxC), las plantas que se comprometen a entregar su ENFICC reciben un pago que les permite mantenerse disponibles. Esto garantiza el suministro incluso en años de sequía extrema.',
  ...
  PerdidasEner: {
    ...
    descripcion: 'La energía que se pierde durante el transporte desde las plantas generadoras hasta los puntos de consumo, principalmente por el calentamiento de los cables (efecto Joule) en el Sistema de Transmisión Nacional.',
    contexto: 'Las pérdidas son inevitables en todo sistema eléctrico, pero se minimizan usando altas tensiones (230–500 kV). Las pérdidas en distribución (redes locales) son adicionales y mayores. Reducirlas es clave para la eficiencia energética.',
  ...
  GeneIdea: {
    ...
    contexto: 'Al comparar la Generación Ideal con la Generación Real se puede cuantificar el impacto de las restricciones de red. Cuando difieren mucho, significa que hay plantas que quieren generar (generalmente más baratas o renovables) pero no pueden porque la línea no da abasto.',
...
function Sparkline({ datos, barClass, height = 'h-10' }) {
  ...
  if (vals.length === 0) return <div className={`${height} flex items-center justify-center text-xs text-gray-300`}>Sin datos</div>;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5 mb-3 pr-6">
        ...
        <div>
          <p className="text-sm font-semibold text-gray-800 leading-tight">{indicador.label}</p>
      ...
      {indicador.error ? (
        ...
        <>
          ...
          <div className="flex items-center gap-1.5 mb-3">
            ...
            <span className={`text-xs font-medium ${variacionNula ? 'text-gray-400' : variacionPositiva ? 'text-emerald-600' : 'text-rose-600'}`}>
              {variacionNula
                ? 'Sin variación'
          ...
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs text-gray-500">
              Prom.: <span className="font-medium text-gray-700">{formatValor(promedio, indicador.unidad)}</span>
            </p>
            <span className="text-xs text-gray-400">{indicador.datos?.length ?? 0} días</span>
          </div>
...
function MetricDetail({ indicador, onClose }) {
  ...
  const cfg  = open ? (COLOR_CONFIG[indicador.color] || COLOR_CONFIG.emerald) : COLOR_CONFIG.emerald;
          ...
          <>
            ...
            <div className={`${cfg.bg} px-6 py-5 text-white`}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs text-white/70 uppercase tracking-wide font-medium">Indicador</p>
                    <h2 className="text-lg font-bold leading-tight">{indicador.label}</h2>
                  </div>
                </div>
                <button
                  onClick={onClose}
              ...
              {!indicador.error && (
                <div className="bg-white/10 rounded-xl px-4 py-3 mt-1">
                  <p className="text-3xl font-bold">{formatValor(ultimo, indicador.unidad)}</p>
                  <div className="flex items-center justify-between mt-1">
                    <p className="text-sm text-white/70">
                      {fechaUltimo ? formatFecha(fechaUltimo) : 'Sin fecha'}
                    </p>
                    {!variacionNula && (
                      <span className={`text-sm font-semibold ${variacionPositiva ? 'text-emerald-200' : 'text-rose-200'}`}>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              ...
              {!indicador.error && indicador.datos?.length > 0 && (
                <div>
                  ...
                  <div className={`${cfg.light} rounded-xl p-3`}>
                    <Sparkline datos={indicador.datos} barClass={cfg.bar} height="h-20" />
                  </div>
                  <p className="text-xs text-gray-400 mt-1.5 text-right">
                    Prom. período: <span className="font-medium text-gray-600">{formatValor(promedio, indicador.unidad)}</span>
                  </p>
                </div>
              )}
              ...
              {info ? (
                <>
                  <div>
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">¿Qué es?</p>
                    <h3 className="text-base font-bold text-gray-800 mb-2">{info.titulo}</h3>
                    <p className="text-sm text-gray-600 leading-relaxed">{info.descripcion}</p>
                  </div>

                  <div className={`${cfg.light} border ${cfg.border} rounded-xl p-4`}>
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Contexto del mercado</p>
                    <p className="text-sm text-gray-700 leading-relaxed">{info.contexto}</p>
                  </div>

                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                    <p className="text-sm text-amber-800 leading-relaxed">{info.dato}</p>
                  </div>
                </>
              ) : (
                <p className="text-sm text-gray-500">Sin descripción disponible para esta métrica.</p>
              )}

              {/* Unidad */}
              <div className="border-t border-gray-100 pt-4">
                <p className="text-xs text-gray-400">
                  <span className="font-semibold">Unidad:</span> {indicador.unidad} ·{' '}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h1 className="text-2xl font-bold">Indicadores del Mercado Eléctrico</h1>
              </div>
              <p className="text-slate-300 text-sm max-w-xl">
                Datos del Sistema Interconectado Nacional (SIN) de Colombia, provistos por{' '}
                <a href="https://sinergox.xm.com.co" target="_blank" rel="noopener noreferrer" className="text-emerald-400 hover:underline">
                  XM S.A. E.S.P.
                </a>
                {' '}· <span className="text-slate-400">Haz clic en cualquier tarjeta para más información</span>
              </p>
            </div>
            {lastUpdate && (
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
            ...
            {[7, 14, 30].map(d => (
              <button
                ...
                className={`px-4 py-2 text-sm font-medium transition-colors ${
                  rangoDias === d ? 'bg-slate-800 text-white' : 'text-gray-600 hover:bg-gray-50'
                }`}
              >
                {d} días
              </button>
        ...
        {!loading && error && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-6 flex flex-col items-center gap-3 text-center">
            ...
            <div>
              <p className="font-semibold text-red-700">No se pudo cargar los indicadores</p>
              <p className="text-sm text-red-500 mt-1">{error}</p>
            </div>
            <button onClick={() => fetchTodos(rangoDias)} className="mt-1 px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-medium hover:bg-red-700 transition">
              Reintentar
            </button>
          </div>
        )}
        ...
        {!loading && (
          <div className="mt-8 bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm text-slate-600">
            ...
            <p>
              Datos provistos por{' '}
              <a href="https://sinergox.xm.com.co" target="_blank" rel="noopener noreferrer" className="text-emerald-600 hover:underline font-medium">
                SINERGOX — XM S.A. E.S.P.
              </a>
              , operador del SIN y administrador del Mercado de Energía Mayorista de Colombia.
              API documentada en{' '}
              <a href="https://github.com/EquipoAnaliticaXM/API_XM" target="_blank" rel="noopener noreferrer" className="text-emerald-600 hover:underline font-medium">
