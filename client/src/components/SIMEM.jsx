...
const TIPOS_RENOVABLES = new Set([
  "Solar",
  "Eólica",
  "Hidráulica",
  "Filo de Agua",
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 py-12 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-white mb-3">
            Mercado de Energía <span className="text-emerald-400">SIMEM</span>
          </h1>
          ...
          datos?.kpis && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
                ...
                <span
                  className={`inline-block mt-2 text-xs px-2 py-1 rounded-full font-medium ${cfg(datos.kpis.fuenteDominante).badge}`}
                >
                  {TIPOS_RENOVABLES.has(datos.kpis.fuenteDominante)
                    ? "Renovable"
                    : "No Renovable"}
                </span>
              </div>
            ...
            {vistaActiva === "mix" && (
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                ...
                <p className="text-sm text-slate-500 mb-6">
                  Distribución acumulada por tipo de fuente · últimos{" "}
                  {rangoDias} días
                </p>
                    ...
                    <div key={item.tipo}>
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-2">
                          <span
                            className={`inline-block w-3 h-3 rounded-full ${cfg(item.tipo).bg}`}
                          />
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <span
                            className={`text-sm font-bold ${cfg(item.tipo).text}`}
                          >
                            {item.pctDelTotal}%
                          </span>
                        </div>
                ...
                <div className="mt-8">
                  ...
                  <div className="flex flex-wrap gap-3 mt-3">
                    {datos.resumenPorTipo.map((item) => (
                      ...
                      >
                        <span
                          className={`w-2.5 h-2.5 rounded-sm ${cfg(item.tipo).bg}`}
                        />
                        {item.tipo}
                      </span>
                    ))}
                    ...
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200">
                        ...
                        {datos.resumenPorTipo.map((item) => (
                          <th
                            key={item.tipo}
                            className="text-right px-4 py-3 font-semibold text-slate-700 whitespace-nowrap"
                          >
                            <span className="flex items-center justify-end gap-1.5">
                              <span
                                className={`w-2 h-2 rounded-full ${cfg(item.tipo).bg}`}
                              />
                              {item.tipo}
                            </span>
                          </th>
                      ...
                      {[...datos.mixPorDia].reverse().map((dia, idx) => (
                        ...
                        >
                          ...
                          {datos.resumenPorTipo.map((item) => (
                            <td
                              key={item.tipo}
                              className={`px-4 py-3 text-right ${cfg(item.tipo).text} font-medium tabular-nums`}
                            >
        ...
        {!loading && !error && datos && (
          <div className="mt-8 p-4 bg-blue-50 border border-blue-200 rounded-xl flex items-start gap-3">
            ...
            <p className="text-blue-700 text-sm">
              ...
              Colombia. Dataset{" "}
              <code className="bg-blue-100 px-1 rounded text-xs">E17D25</code> —
